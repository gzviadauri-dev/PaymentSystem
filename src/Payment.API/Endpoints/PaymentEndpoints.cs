...
public static class PaymentEndpoints
{
    ...
    public static void MapPaymentEndpoints(this WebApplication app)
    {
        ...
        // ── POST /api/payments/{paymentId}/redirect ─────────────────────────
        // Lock the payment to a provider before redirecting the user to the bank.
        // Must be called before the user is sent to the bank's payment page.
        // Returns 409 if the payment is already locked to a different provider.
        group.MapPost("/{paymentId:guid}/redirect", async (
            Guid paymentId,
            RedirectRequest req,
            ...
            CancellationToken ct) =>
        {
            LockProviderResult lockResult;
            try
            {
                lockResult = await paymentRepository.LockProviderAsync(paymentId, req.ProviderId, ct);
            }
            ...
            switch (lockResult)
            {
                case LockProviderResult.Locked:
                case LockProviderResult.AlreadyLockedSame:
                    break; // proceed with redirect

                case LockProviderResult.AlreadyLockedOther:
                    logger.LogWarning(
                        "Payment {PaymentId} is already locked to a different provider. " +
                        "Rejecting redirect to {ProviderId}.",
                        paymentId, req.ProviderId);
                    return Results.Conflict(new
                    {
                        error = "payment_locked_to_other_provider",
                        message = "This payment is already being processed by another provider."
                    });
            }

            // Return the redirect URL (in production, generated by the provider SDK).
            // The paymentId is embedded in the redirect URL so the bank can include it
            // in the callback — enabling the confirm endpoint to filter by Id.
            return Results.Ok(new
            {
                paymentId,
                providerId = req.ProviderId,
                redirectUrl = $"{req.ProviderBaseUrl}/pay?ref={paymentId}",
                locked = lockResult == LockProviderResult.Locked ? "new" : "idempotent"
            });
        })
        .WithName("RedirectToProvider");
...
public record RedirectRequest(string ProviderId, string ProviderBaseUrl);
